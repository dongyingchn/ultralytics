...
def gen_val_data_list():
    ...
    for cate_name in cate_list:
        ...
        if cate_name not in ["driving", "parking"]:
            continue

        cate_dir = os.path.join(data_dir, cate_name)
        save_cate_dir = os.path.join(save_dir, cate_name)
        os.makedirs(save_cate_dir, exist_ok=True)
        
        img_dir = os.path.join(cate_dir, "images")

        img_list = os.listdir(img_dir)
        img_list.sort()
        img_path_list = [os.path.join(img_dir, img_name)+'\n' for img_name in img_list]
        
        save_date_file = os.path.join(save_cate_dir, f"{cate_name}.txt")
...
def load_all_labels_from_list(file_list):
    ...
    df.fillna({'class_name': 'Unknown'}, inplace=True)
...
def cache_gt_statistics(mode='train', chunk_size=10):
    ...
    if mode == 'train':
        ...
        for cate_name in cate_list:
            ...
            date_files = sorted(glob.glob(os.path.join(cate_dir, '*.txt')))
            
            if not date_files:
                continue

            # 准备缓存输出目录
            cache_vehicle_dir = os.path.join(cache_dir, cate_name)
            os.makedirs(cache_vehicle_dir, exist_ok=True)
            ...
            for i, chunk in enumerate(file_chunks):
                # 为每个块生成一个独立的DataFrame
                # 注意：这里我们调用一个新函数，它直接处理文件列表
                print(f"Processing chunk {i+1}/{len(file_chunks)} for {cate_name}...")
                df_chunk = load_all_labels_from_list(chunk)
                ...
                if len(chunk) > 1:
                    ...
                    chunk_start = chunk[0].split('/')[-1].replace('.txt', '')
                    cache_file_name = f"{chunk_start}.pkl"
...
def load_all_cached_labels(cache_dir):
    ...
    for pkl_path in tqdm(all_pkl_files[:], desc="正在加载缓存文件"):
        try:
            ...
            if '/train/' in pkl_path:
                key = pkl_path.split('/')[-3]
            elif '/val/' in pkl_path:
                key = pkl_path.split('/')[-2]
...
def plot_z_distribution(df, output_dir, target_class=None, class_names=None, key=None):
    ...
    if key == 'all-scene' and class_name == 'all-class':
        print("\nZ轴距离分布统计结果:")
        print(counts_df.to_string(index=False))
        # print(f"总3D目标数量: {total_3d_count}")
        # 打印占比
        counts_df['percentage'] = counts_df['count'] / total_3d_count * 100
        print("\n各距离区间占比:")
        print(counts_df[['z_interval', 'percentage']].to_string(index=False, float_format='%.2f%%'))

    # 6. 美化图表
    ax.set_title(f'z_dist_{key}_{class_name} (total: {total_3d_count})', fontsize=20, pad=20)
...
def main():
    ...
    try:
        # 加载所有缓存数据
        df_dict = load_all_cached_labels(CONFIG['cache_dir'])
        print(f"\n成功从缓存中加载数据。")
    except (FileNotFoundError, ValueError) as e:
        print(f"错误: {e}")
        return
    
    for key, combined_df in df_dict.items():
        print(f"\n=== 分析类别: {key} ===")

        if '/train/' in CONFIG['cache_dir']:
            if key not in ['all-scene']:
                continue
        elif '/val/' in CONFIG['cache_dir']:
            if key not in ['driving', 'parking', 'all-scene']:
                continue
