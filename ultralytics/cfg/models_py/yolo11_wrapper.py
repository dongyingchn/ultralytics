...
from ultralytics.cfg.models_py.yolo11 import YOLO11  # 你的 Python 版 YOLO11


class YOLO11DetectionModel(BaseModel):
    """
    ...
    def __init__(self, args, nc: int, ch: int, scale: str = "s"):
        """
        Args:
        ...
        self.yaml = {"channels": ch, "nc": nc}

        # 1. 实例化你自己的 YOLO11
        #    假设 YOLO11(nc=..., ch=..., scale=...) 可用；如果你的 __init__ 没有 ch 参数，请去掉 ch=ch。
        core = YOLO11(nc=nc, ch=ch, scale=scale)

        # 2. 取出 backbone, head, detect
        self.backbone = core.backbone
        self.head = core.head
        self.detect = core.detect  # Detect3D / Detect_with_3D / Detect
        ...
        self.stride = getattr(core, "stride", torch.tensor([8.0, 16.0, 32.0]))
        # enable_3d：让 DetectionModel.init_criterion 风格的逻辑可重用
        self.enable_3d = isinstance(self.detect, type(getattr(self.detect, "__class__", None))) or self.args.get("train_3d", False)
        ...
        self.end2end = getattr(self.detect, "end2end", False)

        # 6. 如果 Detect 头需要 bias_init，并且 stride 已正确初始化，可以在此调用一次
        if hasattr(self.detect, "bias_init") and hasattr(self.detect, "stride"):
            # 防御：确保 stride 没有 0
            if torch.all(self.detect.stride > 0):
...
class QATYOLO11DetectionModel(YOLO11DetectionModel):
    """
    用于量化感知训练的 YOLO11 检测模型包装类。
    继承自 YOLO11DetectionModel，保持相同接口。
    """
    ...
    def forward(self, x, *args, **kwargs):
        ...
        if isinstance(output, (list, tuple)):
            for i in range(len(output)):
                if isinstance(output[i], (list, tuple)):
                    for j in range(len(output[i])):
                        output[i][j] = self.dequant(output[i][j])
                else:
                    output[i] = self.dequant(output[i])
        else:
